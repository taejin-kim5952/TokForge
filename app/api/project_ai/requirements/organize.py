"""요구사항정의서 — 대화 정리(organize) 엔드포인트.

현재 활성 대화의 메시지를 분석해 (헤더 + 표 행) 형태의 구조화된 마크다운 출력.
...
from .prompts import REQUIREMENTS_ORGANIZER_PROMPT, parse_requirements_markdown
...
@router.post("/{project_id}/ai/requirements/organize")
async def organize_requirements(
    project: OwnedProject,
    user: CurrentUser,
    payload: OrganizeRequest,
) -> dict:
    """대화 → (헤더, 요구사항 표 행 배열) 구조화."""
    ...
    system_prompt = (
        prompt_repo.get_active("requirements_organizer") or REQUIREMENTS_ORGANIZER_PROMPT
    )
    ...
    ollama_request = {
        ...
        "messages": [
            ...
            *messages,
            {
                "role": "user",
                "content": "위 대화를 위 규칙대로 (헤더 + 요구사항 표) 마크다운 형식으로 정리해줘.",
            },
        ],
        "stream": False,
        "temperature": 0.3,
    ...
    try:
        ...
        logger.exception("requirements organizer LLM call failed")
    ...
    header, rows = parse_requirements_markdown(content)
